module.exports.config = {
    name: "newworld",
    version: "1.0.0",
    role: 0,
    credits: "Rized",  // Updated credit name
    description: "Generate a New World-themed image using Facebook UID and text",
    hasPrefix: false,
    aliases: ["newworld"],
    usage: "[newworld <uid> | <text>]",
    cooldown: 5
};

const axios = require("axios");
const fs = require("fs");
const path = require("path");

module.exports.run = async function({ api, event, args }) {
    try {
        // Join arguments into a single string
        const input = args.join(" ");

        // Validate input format
        if (!input.includes("|")) {
            api.sendMessage("Usage: newworld <uid> | <text>\nExample: newworld 1000123456789 | Hello world", event.threadID);
            return;
        }

        // Extract UID and text
        const [uid, ...textParts] = input.split("|").map(part => part.trim());
        const text = textParts.join(" ");

        // Validate UID and text
        if (!uid || !text || isNaN(uid)) {
            api.sendMessage("Invalid input. Make sure the UID is numeric and text is provided.\nUsage: newworld <uid> | <text>", event.threadID);
            return;
        }

        // Construct the API URL with properly encoded parameters
        const encodedText = encodeURIComponent(text);
        const url = `https://betadash-api-swordslush-production.up.railway.app/new-world?userid=${uid}&text=${encodedText}`;
        const imagePath = path.join(__dirname, "newworld.png");

        // Notify the user that the image is being generated
        api.sendMessage("Generating your New World image, please wait...", event.threadID);

        // Fetch the image from the API
        const response = await axios({
            url: url,
            method: 'GET',
            responseType: 'stream'
        });

        // Create a writable stream to save the image
        const writer = fs.createWriteStream(imagePath);
        response.data.pipe(writer);

        // Handle the finish event of the writable stream
        writer.on('finish', async () => {
            try {
                // Send the image as an attachment
                await api.sendMessage({
                    attachment: fs.createReadStream(imagePath)
                }, event.threadID);

                // Clean up the file after sending
                fs.unlinkSync(imagePath);
            } catch (sendError) {
                console.error('Error sending image:', sendError);
                api.sendMessage("An error occurred while sending the image.", event.threadID);
            }
        });

        // Handle errors during the writing process
        writer.on('error', (err) => {
            console.error('Stream writer error:', err);
            api.sendMessage("An error occurred while processing the request.", event.threadID);
        });

    } catch (error) {
        console.error('Error:', error);
        api.sendMessage("An error occurred while processing the request.", event.threadID);
    }
};