...
module.exports.run = async function({ api, event, args }) {
    try {
        ...
        if (!text) {
            api.sendMessage("Usage: alert <text>", event.threadID);
            return;
        }

        // Construct the API URL with properly encoded parameters
        const encodedText = encodeURIComponent(text);
        const url = `https://api.popcat.xyz/v2/alert?text=${encodedText}`;
        const imagePath = path.join(__dirname, "alert.png");

        // Notify the user that the image is being generated
        api.sendMessage("🚨 Generating your alert image, please wait...", event.threadID);
