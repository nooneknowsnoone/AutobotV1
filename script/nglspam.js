...
module.exports.run = async function({ api, event, args }) {
  ...
  api.sendMessage(waitingMessage, event.threadID, async (err, info) => {
    ...
    try {
      const apiUrl = `https://xvi-rest-api.vercel.app/api/nglspam?username=${encodeURIComponent(username)}&amount=${encodeURIComponent(amount)}&message=${encodeURIComponent(message)}`;
