module.exports.config = {
    ...
    role: 0,
    credits: "developer",
    description: "Generate a caution sign image",
    hasPrefix: false,
    aliases: ["caution"],
    usage: "[caution <text>]",
    cooldown: 5
...
module.exports.run = async function({ api, event, args }) {
    try {
        ...
        if (!text) {
            api.sendMessage("Usage: caution <text>", event.threadID);
            return;
        }

        // Construct the API URL with properly encoded parameters
        const encodedText = encodeURIComponent(text);
        const url = `https://api.popcat.xyz/v2/caution?text=${encodedText}`;
        const imagePath = path.join(__dirname, "caution.png");

        // Notify the user that the image is being generated
        api.sendMessage("⚠️ Generating your caution image, please wait...", event.threadID);
