...
module.exports.config = {
  ...
  version: "1.0.0",
  credits: "Ry",
  description: "Ask anything to Claude 3 Haiku AI",
...
module.exports.run = async function ({ api, event, args }) {
  ...
  api.sendMessage(formatFont("🤖 𝗖𝗟𝗔𝗨𝗗𝗘 𝗜𝗦 𝗣𝗥𝗢𝗖𝗘𝗦𝗦𝗜𝗡𝗚..."), threadID, async (err, info) => {
    if (err) return;

    try {
      const { data } = await axios.get("https://kaiz-apis.gleeze.com/api/claude3-haiku", {
        params: {
          ask: prompt,
      ...
      const reply = data.response || "❌ No response received from Claude.";
