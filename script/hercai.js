...
module.exports.config = {
  ...
  version: "1.0.0",
  credits: "Ry",
  description: "Ask anything to Hercai AI",
  usage: "hercai <question>",
  cooldown: 3,
  role: 0,
  hasPrefix: false,
  aliases: [],
...
module.exports.run = async function ({ api, event, args }) {
  ...
  api.sendMessage(formatFont("🤖 𝗛𝗘𝗥𝗖𝗔𝗜 𝗜𝗦 𝗧𝗛𝗜𝗡𝗞𝗜𝗡𝗚..."), threadID, async (err, info) => {
    if (err) return;

    try {
      const { data } = await axios.get(`https://www.smfahim.xyz/hercai?ask=${encodeURIComponent(prompt)}`);
      const reply = data.answer?.trim() || "❌ No response received from Hercai.";
...
⏰ 𝗧𝗶𝗺𝗲: ${timePH}`.trim();
      ...
      console.error("Hercai Error:", err);
