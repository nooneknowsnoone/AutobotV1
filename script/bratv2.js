module.exports.config = {
    ...
    description: "Generate a bratv2 image with your custom text",
    hasPrefix: false,
    aliases: ["brat"],
    usage: "[bratv2 <text>]",
    cooldown: 5
...
module.exports.run = async function({ api, event, args }) {
    try {
        ...
        if (!text) {
            api.sendMessage("Usage: bratv2 <text>", event.threadID);
            return;
        }

        // Construct the API URL with properly encoded parameters
        const encodedText = encodeURIComponent(text);
        const url = `https://api.ferdev.my.id/maker/bratv2?text=${encodedText}`;
        const imagePath = path.join(__dirname, "bratv2.png");

        // Notify the user that the image is being generated
        api.sendMessage("Generating your bratv2 image, please wait...", event.threadID);
