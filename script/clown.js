module.exports.config = {    
    ...
    role: 0,    
    credits: "Rized",    
    description: "Generate a Clown-themed image using a user ID",    
    hasPrefix: false,    
    aliases: ["clown"],    
    usage: "[clown <userid>]",    
    cooldown: 5    
...
module.exports.run = async function({ api, event, args }) {    
    try {    
        ...
        const url = `https://betadash-api-swordslush-production.up.railway.app/clown?userid=${userId}`;    
        const imagePath = path.join(__dirname, `clown_${Date.now()}.png`);    
    
        // Notify the user    
        api.sendMessage("Generating Clown image, please wait...", event.threadID);    
