...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "Generate a 'drip' meme pretending to be rich using someone's profile picture.",
  cooldown: 5,
  hasPrefix: true,
...
module.exports.run = async function ({ api, event, args }) {
  ...
  const outputPath = __dirname + `/cache/drip_${userID}_${targetID}.png`;
  ...
  try {
    ...
    let dripURL = `https://api.popcat.xyz/drip?image=${profileURL}`;
    ...
    writer.on('finish', () => {
      api.sendMessage({
        body: formatFont(`${name} is flexing rich drip jacket! 💸✨`),
