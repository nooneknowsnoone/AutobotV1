...
module.exports.config = {
  ...
  hasPrefix: false,
  aliases: [],
  description: "Simple AI response using LearnLM API",
  usage: "learn [prompt]",
  credits: 'Ry',
  cooldown: 3,
};
...
module.exports.run = async function ({ api, event, args }) {
  ...
  api.sendMessage(formatFont('📚 𝗟𝗘𝗔𝗥𝗡𝗟𝗠 𝗜𝗦 𝗧𝗛𝗜𝗡𝗞𝗜𝗡𝗚...'), threadID, async (err, info) => {
    if (err) return;

    try {
      const { data } = await axios.get("https://wildan-suldyir-apis.vercel.app/api/learnlm", {
...
⏰ 𝗧𝗶𝗺𝗲: ${timePH}
      ...
      console.error("LearnLM API Error:", err);
